...
Tous les docs/cleanup faits : README, ARCH, CLAUDE, formats, marketing/README, prompts, la-toile, repoint marketing-fr/en, archivage system-prompts, identité Jarvis. **Prochaine étape : BLOC 4** — plan chirurgical créé le 08/06 : `BLOC4-FRONTEND-PLAN.md` (B4.1 targets.ts [retrait f2 + canon 41/82] → B4.2 retrait toggle → B4.3 démêlage mode f2 ~12 composants → B4.4 labels F2 → B4.5 archivage ui/web → B4.6 scalabilité multi-business [STATUS.md en contextPaths + design sélecteur entité] → B4.7 docs Mode F2). Audit de cadrage fait sur repo36 : suite `/api/action/*` + 2 interfaces R/F + CounterTile DÉJÀ construits (hors scope) ; le vrai chantier = mode f2 tissé dans ~12 composants + targets.ts encore f2. Décisions actées : ui/web ARCHIVÉ ; wings mémoire f2 = affichage CONSERVÉ ; business = propriété du contenu, pas un mode UI. **08/06 — 3 décisions du plan toutes tranchées** : canon compteurs = ligne chat.ts l.318 (Cold TikTok/10 · Insta/10 · FB/5 · TW/5 · LI/5 + PH/6 = 41/persona, 82 général) ; items F2 historiques affichés sur les 2 cockpits (timeline commune), attribution F2→Romain gardée, seul le toggle retiré ; sélecteur business à la sortie d'Hokuno ; dette daily-checklist rayée (fichier absent). Plan sans question ouverte. **B4.1 ✅ (repo37)** : targets.ts réécrit (cibles canon fixes 41/82, f2 + parsing batch supprimés) + compteurs cockpit réalignés (mobile+desktop, Total persona /41 + Général R+F /82, bug tuiles-à-0 corrigé), typecheck 0 erreur, confirmé visuellement par R. **B4.2 ✅ (repo38)** : toggle ROMAIN/F2 + bannière F2Banner + état f2Mode entièrement retirés du cockpit (PersonaLayout 16 zones + context-types + app/romain), 9 props enfants à mode="normal" en attendant B4.3, typecheck 0 erreur, disparition confirmée visuellement par R. **B4.3 ✅ (repo39)** : prop mode f2 démêlé des 9 composants (Chat/BatchCard/TimelineColumn/FileViewerModal/Brain3D×2/RepoGraph3D×2/PersonaLayout-AlertRow + 9 câbles), fonction morte filterBatchByPersona supprimée, historique chat préservé (défaut backend), garde-fous respectés (attribution F2→Romain + wings mémoire). Typecheck 0 erreur. **B4.4 ✅ (repo40)** : 4 labels « F2 » → FoundryTwo (GraphifyView ×2, GraphifyFullscreen, brain-3d zones.ts). **B4.5 ✅ (repo41)** : ui/web archivé en 07-tech-legacy (git mv, 14 fichiers) + 3 refs doc corrigées. **B4.6a ✅ (repo42, 12/06)** : STATUS.md réécrit (4 business canon : StoreMD live, ProfitPilot, Hokuno [dissocié d'Otaku-Store le 12/06, aucun cold], Otaku-Store ; vivier 5 SaaS en trace) + câblé en 6e fichier des contextPaths chat.ts. **B4.6b ✅ (12/06) : design sélecteur business figé (canon au RECAP §8, implémentation à Hokuno).** **B4.7 ✅ (repo43, 12/06)** : docs Mode F2 nettoyées (5 fichiers : manuel + architecture corrigés avec pierres tombales datées, plan historique annoté, skill jarvis-upgrade purgé de la couleur F2, README cockpit). **CONSTRUCTION BLOC 4 TERMINÉE (B4.1→B4.7)**. **⚡ CHECK BRANCHEMENTS — fils code ✅ (repo44, 12/06)** : balayage exhaustif fait (1 faux positif écarté : cognitive_load sain) ; 2 fils repointés en 6 retouches/5 fichiers (batch : batch-number.ts + batch.ts + prompt chat.ts + bouton Posts batchés → `marketing/contenu/batch-semaine/batch-semaine-SN.md` ; pipeline Fabrice ×3 → `marketing/saas-app-shopify/storemd/pipeline-conversion.md`). **Diagnostic runtime fait (navigateur)** : `/api/graph` → « Graph build failed — check GITHUB_TOKEN » · `/api/ouroboros/status` → initialized:false alors que state.json existe (249 cycles) → **cause unique probable : GITHUB_TOKEN Railway expiré** (explique aussi compteurs à 0). **Cause confirmée et réparée (12/06 soir)** : token GitHub `jARVIS` expiré le 20/05 (= dernier cycle Ouroboros). Nouveau token (classic, scope repo, no expiration) posé dans Railway par R → redéploiement → **confirmation visuelle : tout connecté**. Purge volontaire R des 45 proposals pending périmées (S9/S10). **🔒 BLOC 4 CLOS (12/06/2026). Baseline finale : repo45.** Le branchement Jarvis (Blocs 0→4) est TERMINÉ ; restes au RECAP §8 (points ouverts) et §9 (dettes).
