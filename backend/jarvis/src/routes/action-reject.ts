...
import { rejectAction } from "../lib/action-executor.js";
import { getSupabase } from "../lib/supabase.js";

export async function actionRejectRoute(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const { data: row, error: readErr } = await sb
      ...
      .select("id, jarvis_conversations!inner(user_id)")
      .eq("id", action_id)
      .single();
    if (readErr || !row) {
